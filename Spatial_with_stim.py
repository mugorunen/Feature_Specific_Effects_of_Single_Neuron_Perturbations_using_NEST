...
def calculate_avg_firing(pop, sim_time, spikes, times):
    min_value = np.min(spikes)
    max_value = np.max(spikes)
    neuron_ids = [i for i in range(min_value, max_value + 1, 1)]
    firing_rates = []
    spike_times = []
    for neuron_id in neuron_ids:
        spikes_for_neuron = np.sum(spikes == neuron_id)
        firing_rate = spikes_for_neuron / (sim_time / 1000)  # Convert to Hz
        #if neuron_id == src_id or neuron_id == inh_id1 or neuron_id == inh_id2:
        if neuron_id == src_id or neuron_id==exc_id1:
            print(neuron_id)
        else:
...
avg_firing_exc, spike_times_exc = calculate_avg_firing(nodes_ex, simtime, sr1_spikes, sr1_times)
print(avg_firing_exc)
# Calculate avg. firing rates of inhibitory neurons
avg_firing_inh, spike_times_inh = calculate_avg_firing(nodes_in, simtime, sr2_spikes, sr2_times)
print(avg_firing_inh)

print(len(spike_times_exc[0]))
print(type(spike_times_exc[0]))
print(spike_times_exc[0].shape)
print(type(spike_times_exc[0][0]))
print(spike_times_exc[0][0].shape)
...
time_axis = np.linspace(0, 20000, len(average_firing_rate))
